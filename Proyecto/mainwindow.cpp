    ...
    , ui(new Ui::MainWindow)
{
    ...
    // Conectar los nuevos botones de DFS y BFS con sus respectivos slots
    connect(ui->dfsButton, &QPushButton::clicked, this, &MainWindow::on_dfsButton_clicked);
    connect(ui->bfsButton, &QPushButton::clicked, this, &MainWindow::on_bfsButton_clicked);
}
...
Neurona* MainWindow::findNeuronById(int id) {
    ListaNeuronas& lista = this->lista;
    for (Neurona& neurona : lista.getLista()) {
        if (neurona.get_id() == id) {
            return &neurona;
        }
    }
    return nullptr;
...
void MainWindow::depthFirstSearch(int startId) {
    Neurona* startNeuron = findNeuronById(startId);
    if (!startNeuron) {
        ui->resultTextEdit->setText("Nodo inicial no encontrado.");
        return;
    }
    ...
    while (!stack.isEmpty()) {
        ...
        if (!visited.contains(currentId)) {
            ...
            Neurona* currentNeuron = findNeuronById(currentId);
            result += QString::number(currentId) + " ";

            for (auto &neighbor : lista.getLista()) {
                if (!visited.contains(neighbor.get_id()) && isConnected(*currentNeuron, neighbor)) {
                    stack.push(neighbor.get_id());
                }
            }
        }
    }
...
void MainWindow::breadthFirstSearch(int startId) {
    Neurona* startNeuron = findNeuronById(startId);
    if (!startNeuron) {
        ui->resultTextEdit->setText("Nodo inicial no encontrado.");
        return;
...
void MainWindow::on_dfsButton_clicked() {
    int startNode = ui->startNodeSpinBox->value();
...
bool MainWindow::isConnected(const Neurona& neurona1, const Neurona& neurona2) {
    double distance = std::sqrt(std::pow(neurona1.get_posicionX() - neurona2.get_posicionX(), 2) +
                                std::pow(neurona1.get_posicionY() - neurona2.get_posicionY(), 2));
    return true;
}
